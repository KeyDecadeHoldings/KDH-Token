...
if (cliOptions.help) {
  ...
  // Config
  const solidityFile = './smart_contract/KeyDecadeHoldings.sol';
  const contractName = 'KeyDecadeHoldings';
  const KDHVersion = 'v0.4.9+commit.364da425';
  ...
  const feeMake = 0;
  const feeTake = 30000000; 
  const feeRebate = 0;
  const gas = 2000000;
  const args = [admin, feeAccount, accountLevelsAddr, feeMake, feeTake, feeRebate];

  KDH.loadRemoteVersion(KDHVersion, (err, KDHV) => {
    console.log('KDH version:', KDHV.version());
    fs.readFile(solidityFile, (errRead, result) => {
      const source = result.toString();
      const output = KDHV.compile(source, 1); // 1 activates the optimiser
